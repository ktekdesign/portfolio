export const servicesData = [
    {
      iconName: "pe-7s-display2 bg-6610f2",
      shortText:
        "À la pointe de la technologie, nous avons une longueur d’avance en matière de référencement, d’optimisation et de sécurité.",
      viewDetails: "/service/creation-site-web/",
      aosDelay: "200",
      pageTitle: "Création de site web",
      BGImage: "/images/hero-banner3.jpg",
      method: "<p>Nos services de création de sites web comprennent :</p> \
        <ul> \
        <li>Analyse des besoins : Nous travaillons avec vous pour comprendre vos objectifs commerciaux et identifier les fonctionnalités clés de votre site web</li> \
        <li>Conception : Nous créons des designs d'interface utilisateur attrayants et intuitifs qui offrent une expérience utilisateur exceptionnelle</li> \
        <li>Développement : Nous développons votre site web en utilisant les dernières technologies et les meilleures pratiques pour garantir des performances optimales et une sécurité accrue</li> \
        <li>Tests et déploiement : Nous testons rigoureusement votre site web pour garantir qu'il fonctionne correctement sur toutes les plateformes et appareils, puis nous le déployons sur votre serveur d'hébergement ou sur un service cloud</li> \
        </ul>",
      description: 
        "<p>Vous cherchez à créer un site web pour votre entreprise ou votre projet personnel ? Nous sommes là pour vous aider !</p> \
        <p>Notre équipe de conception et de développement de sites web est spécialisée dans la création de sites web professionnels, attrayants et performants. Nous travaillons avec vous pour comprendre vos objectifs commerciaux et créer un site web qui répondra à vos besoins et attirera votre public cible.</p> \
        <p>Nous sommes passionnés par la création de sites web qui offrent une expérience utilisateur exceptionnelle et qui aident les entreprises à atteindre leurs objectifs en ligne. Nous travaillons avec des clients dans une variété de secteurs d'activité pour créer des sites web qui répondent à leurs besoins spécifiques.</p> \
        <p>Contactez-nous dès aujourd'hui pour découvrir comment nous pouvons vous aider à créer un site web qui impressionnera votre public cible et renforcera votre présence en ligne.</p>"
    },
    {
      iconName: "pe-7s-phone bg-fc3549",
      shortText:
        "Confier le développement de votre application à des spécialistes, capables de combler toutes vos attentes, est en garantir le succès.",
      viewDetails: "/service/application-android-et-ios/",
      aosDelay: "400",
      pageTitle: "Application Android et iOS",
      BGImage: "/images/hero-banner4.jpg",
      method: "<p>Nos services de conception d'applications mobiles comprennent :</p> \
      <ul> \
      <li>Analyse des besoins : Nous travaillons avec vous pour comprendre vos objectifs commerciaux et identifier les fonctionnalités clés de votre application mobile</li> \
      <li>Conception : Nous créons des designs d'interface utilisateur attrayants et intuitifs qui offrent une expérience utilisateur exceptionnelle</li> \
      <li>Développement : Nous développons votre application mobile en utilisant les dernières technologies et les meilleures pratiques pour garantir des performances optimales et une sécurité accrue</li> \
      <li>Tests et déploiement : Nous testons rigoureusement votre application pour garantir qu'elle fonctionne correctement sur toutes les plateformes et appareils, puis nous la déployons sur l'App Store ou le Play Store</li> \
      </ul>",
      description: "<p>Vous cherchez à développer une application mobile pour Android ou iOS ? Nous sommes là pour vous aider !</p> \
        <p>Notre équipe de conception et de développement d'applications mobiles est spécialisée dans la création d'applications de haute qualité pour les deux principales plateformes mobiles. Nous travaillons avec vous pour comprendre les besoins de votre entreprise et créer une application mobile qui répondra à vos attentes et celles de vos utilisateurs.</p> \
        <p>Nous sommes passionnés par la création d'applications mobiles qui offrent une expérience utilisateur exceptionnelle et qui aident les entreprises à atteindre leurs objectifs. Nous travaillons avec des clients dans une variété de secteurs d'activité pour créer des applications mobiles qui répondent à leurs besoins spécifiques.</p> \
        <p>Contactez-nous dès aujourd'hui pour découvrir comment nous pouvons vous aider à concevoir et à développer une application mobile pour Android ou iOS qui impressionnera vos utilisateurs et renforcera votre présence sur le marché.</p>"
    },
    {
      iconName: "pe-7s-cart bg-00d280",
      shortText:
        "Un site e-commerce est le principal allié des commerçants qui décident d’élargir la portée de la divulgation et de la vente de leurs produits.",
      viewDetails: "/service/e-commerce/",
      aosDelay: "500",
      pageTitle: "E-Commerce",
      BGImage: "/images/hero-banner10.jpg",
      method: "<p>Nos services de création de sites e-commerce comprennent :</p> \
      <ul> \
      <li>Analyse des besoins : Nous travaillons avec vous pour comprendre vos besoins commerciaux et identifier les fonctionnalités clés de votre boutique en ligne</li> \
      <li>Conception : Nous créons des designs d'interface utilisateur attrayants et intuitifs qui offrent une expérience d'achat exceptionnelle</li> \
      <li>Développement : Nous développons votre site e-commerce en utilisant les dernières technologies et les meilleures pratiques pour garantir des performances optimales et une sécurité accrue</li> \
      <li>Intégration de la plateforme de paiement : Nous intégrons les meilleures plateformes de paiement pour garantir une expérience de paiement rapide et sécurisée pour vos clients</li> \
      <li>Tests et déploiement : Nous testons rigoureusement votre site e-commerce pour garantir qu'il fonctionne correctement sur toutes les plateformes et appareils, puis nous le déployons sur votre serveur d'hébergement ou sur un service cloud</li> \
      </ul>",
      description: 
        "<p>Vous cherchez à lancer une boutique en ligne pour votre entreprise ou votre marque ? Nous sommes là pour vous aider !</p> \
        <p>Notre équipe de conception et de développement de sites e-commerce est spécialisée dans la création de boutiques en ligne professionnelles, personnalisées et rentables. Nous travaillons avec vous pour comprendre vos besoins commerciaux et créer une plateforme de vente en ligne qui répondra à vos attentes et attirera votre public cible.</p> \
        <p>Nous sommes passionnés par la création de boutiques en ligne qui offrent une expérience d'achat exceptionnelle et qui aident les entreprises à atteindre leurs objectifs en ligne. Nous travaillons avec des clients dans une variété de secteurs d'activité pour créer des boutiques en ligne qui répondent à leurs besoins spécifiques.</p> \
        <p>Contactez-nous dès aujourd'hui pour découvrir comment nous pouvons vous aider à créer une boutique en ligne qui impressionnera votre public cible et augmentera vos ventes en ligne.</p>"
    },
    {
      iconName: "pe-7s-light bg-ffb700",
      shortText:
        "Investissez en action commerciale avec un fort potentiel de gain tout en contribuant à l’édification et au positionnement de votre marque.",
      viewDetails: "/service/marketing-digital/",
      aosDelay: "300",
      pageTitle: "Marketing Digital",
      BGImage: "/images/hero-banner12.jpg",
      method: "<p>Nos services de marketing digital comprennent :</p> \
        <ul> \
        <li>Optimisation des moteurs de recherche (SEO) : Nous améliorons la visibilité de votre site web sur les moteurs de recherche en optimisant le contenu et la structure de votre site web.</li> \
        <li>Publicité en ligne : Nous créons et gérons des campagnes publicitaires en ligne sur des plateformes telles que Google Ads et Facebook Ads pour attirer plus de clients potentiels vers votre entreprise.</li> \
        <li>Marketing par courriel : Nous créons et envoyons des campagnes de marketing par courriel pour rester en contact avec vos clients actuels et potentiels et promouvoir votre entreprise.</li> \
        <li>Marketing des médias sociaux : Nous créons et gérons des campagnes de marketing des médias sociaux pour attirer plus de followers et d'engagement sur les plateformes de médias sociaux.</li> \
        </ul>",
      description: 
        "<p>Vous cherchez à améliorer la visibilité de votre entreprise en ligne et à attirer plus de clients potentiels ? Nous sommes là pour vous aider !</p> \
        <p>Notre équipe de marketing digital est spécialisée dans la création et la mise en œuvre de stratégies de marketing efficaces pour aider les entreprises à atteindre leurs objectifs commerciaux en ligne. Nous travaillons avec vous pour comprendre vos objectifs et votre public cible, puis nous créons une stratégie personnalisée pour répondre à vos besoins spécifiques.</p> \
        <p>Nous sommes passionnés par la création de stratégies de marketing digital efficaces qui aident les entreprises à atteindre leurs objectifs en ligne. Nous travaillons avec des clients dans une variété de secteurs d'activité pour créer des stratégies de marketing personnalisées qui répondent à leurs besoins spécifiques.</p> \
        <p>Contactez-nous dès aujourd'hui pour découvrir comment nous pouvons vous aider à améliorer la visibilité de votre entreprise en ligne et à attirer plus de clients potentiels.</p>"
    },
    {
      iconName: "pe-7s-settings bg-13c4a1",
      shortText:
        "Laissez nous vous montrer comment Amazon Web Services peut mettre votre startup en avant dans votre domaine en combinant securité et haute disponiblité.",
      viewDetails: "/service/devops-solution/",
      aosDelay: "100",
      pageTitle: "DevOps Solution",
      BGImage: "/images/hero-banner14.jpg",
      method: "<p>Nos experts en DevOps vous aideront à :</p> \
      <ul> \
      <li>Mettre en place des processus de développement agiles et des pratiques DevOps</li> \
      <li>Utiliser les derniers outils et technologies pour automatiser votre chaîne de développement</li> \
      <li>Optimiser vos environnements de production pour une performance maximale et une disponibilité élevée</li> \
      <li>Gérer la sécurité de votre application et protéger vos données</li> \
      <li>Évaluer les performances de votre application et les améliorer en continu</li> \
      </ul>",
      description: 
        "<p>Vous êtes à la recherche d'une solution pour accélérer votre cycle de développement et améliorer la qualité de vos logiciels ? Notre équipe de DevOps est là pour vous aider !</p> \
        <p>Nous offrons des services de DevOps personnalisés pour répondre aux besoins de votre entreprise. Nous travaillons en étroite collaboration avec votre équipe de développement pour automatiser le déploiement, les tests, l'intégration continue et la livraison continue, ce qui permet de réduire le temps de mise sur le marché de vos applications et d'augmenter leur qualité.</p> \
        <p>Notre équipe de DevOps possède une expérience approfondie dans la mise en place de solutions DevOps pour une variété de clients dans divers secteurs d'activité. Nous sommes passionnés par l'amélioration des processus de développement et de livraison de logiciels, ce qui vous permettra de vous concentrer sur la création de valeur pour vos clients.</p> \
        <p>Contactez-nous dès aujourd'hui pour découvrir comment nous pouvons vous aider à accélérer votre cycle de développement et à améliorer la qualité de vos logiciels avec notre service de DevOps personnalisé.</p>"
    },
    
    {
      iconName: "pe-7s-cloud bg-ff612f",
      shortText:
        "Vous avez déjà créé votre site et tout fonctionne parfaitement sur votre ordinateur? Le monde entier mérite d'apprécier votre savoir-faire. Allons-y!",
      viewDetails: "/service/hebergement-web/",
      aosDelay: "600",
      pageTitle: "Hébergement web",
      BGImage: "/images/hero-banner9.jpg",
      method: "<p>Nos services d'hébergement web comprennent :</p> \
      <ul> \
      <li>Hébergement partagé : Idéal pour les sites web de petite à moyenne taille qui ne nécessitent pas de ressources importantes</li> \
      <li>Hébergement WordPress : Spécialement conçu pour les sites web WordPress, avec des fonctionnalités optimisées pour la sécurité et les performances</li> \
      <li>Serveurs VPS : Pour ceux qui ont besoin de plus de flexibilité et de contrôle sur leur environnement d'hébergement</li> \
      <li>Serveurs dédiés : Pour les applications d'entreprise qui ont besoin de ressources dédiées et d'une sécurité accrue</li> \
      </ul>",
      description: "<p>Vous cherchez un hébergement web fiable pour votre site ou votre application en ligne ? Nous avons la solution pour vous ! </p> \
      <p>Nous offrons des services d'hébergement web de qualité supérieure pour répondre à vos besoins spécifiques. Que vous cherchiez un hébergement partagé pour un petit site web ou un serveur dédié pour une application d'entreprise, notre équipe d'experts est là pour vous aider.</p> \
      <p>Notre infrastructure de serveurs est conçue pour offrir une disponibilité maximale et des performances optimales. Nous utilisons des technologies de pointe pour garantir que vos données sont sécurisées et que votre site est toujours disponible pour vos visiteurs.</p> \
      <p>Nous offrons également des services de migration pour transférer votre site existant vers notre plateforme d'hébergement en toute sécurité et sans interruption de service.</p> \
      <p>Notre équipe de support technique est disponible 24h/24 et 7j/7 pour répondre à vos questions et résoudre tout problème que vous pourriez rencontrer. Nous sommes passionnés par la satisfaction de nos clients et nous nous engageons à offrir un service d'hébergement web de première qualité.</p> \
      <p>Contactez-nous dès maintenant pour en savoir plus sur nos services d'hébergement web et découvrir comment nous pouvons vous aider à héberger votre site ou votre application en ligne avec succès.</p>"
    },
  ];